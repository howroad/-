 ...
 */
@Service("codeService")
public class CodeServiceImpl implements ICodeService,Serializable {
	/**
	 * 
	 */
	private static final long serialVersionUID = -4783894855256689730L;
	...
	public BufferedImage getImg() {
		...
		drawRandomLine(g);
		// 6.写在图片上随机数
		codeStr = drawRandomNum((Graphics2D) g, "nl");
	...
	private void setBackGround(Graphics g) {
		// 设置颜色
		g.setColor(new Color(new Random().nextInt(256), new Random().nextInt(256), new Random().nextInt(256), 100));
		// 填充区域
	...
	private String createRandomChar(Graphics2D g, String baseChar) {
		...
		for (int i = 0; i < 4; i++) {
			// 设置颜色
			g.setColor(new Random().nextInt(2) == 0 ? Color.RED : Color.WHITE);
			// 设置字体旋转角度
